...
public class Templates {
	 ...
	 * 
	 * @param rej
	 *            - режим какую папку открывать <br>
	 *            1 - templates 2 - calc
	 * @param typ
	 *            - тип обработки для обработок папки calc <br>
	 * @param curDir
	 *            - или null, или устанавливает текущую папку
	 */
	...
	/**
	 * Устанавливает редактирования папки шаблонов - для запуска
	 * {@link TplEditorActivity}
	 */
			...
			for (int i = 0; i < Instructions.length; i++) {
				...
				if (ff == f + 1) {
					...
					case 13: // selToPos
						...
						// Если отрицательная, то считаем позицию выделения после текущего положения
						// курсора
						// 2 параметр - как искать.
						// Если >=0, то ищем до самого малого значения первого вхождения символов
						// перечисленных
						// в 4 параметре
						// Если <0, то ищем всю строку целиком, указанную в 4 параметре
						// 3 параметр - откуда искать, с начала или конца текста (число + или -).
						// (НЕ ЗАБЫТЬ УКАЗАТЬ ЧТО. ИСХОДНЫЙ ТЕКСТ, ДОЛЖЕН БЫТЬ НЕ БОЛЕЕ 200000
						// символов!)
						// 4 параметр - строка символов, по чему искать.
						// ОБЯЗАТЕЛЬНО ДОЛЖЕН БЫТЬ ПОСЛЕДНИМ, ЧТОБЫ МОЖНО БЫЛО ИСПОЛЬЗОВАТЬ СИМВОЛ
						// ЗАПЯТОЙ!
						int selst = -1;
						int selend = -1;
							...
							if (poz > -1) {
								...
								if (curstr.length() > 0) {
									...
									if(curstr.startsWith(STR_TPL_FOLDER)) {
										curstr = curstr.substring(STR_TPL_FOLDER.length());
//										curstr = st.getSettingsPathFull(curstr, INT_FOLDER_TEMPLATES);
						            	new Templates(1,0,curstr).makeCommonMenu();
								...
								if (pb > -1) {
									...
									for (int iii = 0; iii < ar4.length; iii++) {
										try {
											...
											if (code[iii] <= st.KEYCODE_CODE && code[iii] >= st.KEYCODE_CODE - 2000) {
									...
									if (cod > 0) {
										...
										ServiceJbKbd.inst.sendHardwareSequence(mic, st.KEYCODE_CODE - cod);
									else {
										template_processing = true;
										ServiceJbKbd.inst.onKey(cod, null);
										template_processing = false;
	...
	String getDecodeStringProgramInstruction(String in) {
		...
		if (in.length() < 3)
			...
			if (ff.exists()) {
				...
				if (ff.isDirectory()) {
					in = STR_TPL_FOLDER+ff.getAbsolutePath();//.getName();
					return in;
				}
			} else {
	...
	void makeCommonMenu() {
		...
		for (File f : m_arFiles) {
			if (f.isDirectory()) {
				...
				if ((ServiceJbKbd.inst.m_hotkey_dir).length() > 0 && dir.contains(ServiceJbKbd.inst.m_hotkey_dir)
		...
		st.help = st.c().getString(R.string.mm_path) + st.STR_LF + Templates.inst.m_curDir.getPath() + st.STR_LF
				+ st.STR_LF;
	 ...
	 * 
	 * @param f1
	 *            Позиция символа \r . для bLast=true вычисляется через lastIndexOf,
	 *            для bLast =false - через indexOf
	 * @param f2
	 *            Позиция символа \n. Или наоборот, пофигу
	 * @param bLast
	 *            true - поиск ведется вверх от курсора, false - вниз от курсора
	 * @param len
	 *            Текущая длина строки. Если len<4000 и ни одного переноса строки не
	 *            найдено - для bLast = true вернет 0, для bLast=false - len
	 * @return Возвращает позицию конца или начала строки или -1, если не найдено
	 *         (текст >=4000 символов и в нём нет ни одного переноса)
	 */
	 ...
	 * 
	 * @param seq
	 *            Текст, взятый функцией
	 *            {@link InputConnection#getTextBeforeCursor(int, int)}. Может быть
	 *            null
	 ...
	 *            {@link InputConnection#getTextBeforeCursor(int, int)}. Может быть
	 *            null
	 * @param bRetEmptyIfNotDelimiter
	 *            - true - если не найден конец слова, вернёт пустую строку. false -
	 *            вернёт строку seq
	...
	void setHotDir(String dir) {
		st.pref().edit().putString(st.PREF_KEY_HOT_DIR, m_curDir.getAbsolutePath() + st.STR_SLASH + dir.trim())
				.commit();
	...
	/** константа для специнструкции program */
	public static final String STR_TPL_FOLDER = "$folder:";
	File file;
