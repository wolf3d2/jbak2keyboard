...
//        p = st.pref(this);
        ...
        //setSummary(st.PREF_KEY_CHECK_UPD_APP, R.string.upd_check_desc, strVal(p.getString(st.PREF_AC_HEIGHT,"bbb" )));
...
// выводить экран истории версий, или "как пользоваться клавиатурой"
	 ...
	 * @param type - тип, что выводить <br>
	 * 0 - пустую строку <br>
	 * 1 - есть/нет новая версия + дата последней проверки <br>
	 * 2 - строка Проверяю...
	 * @param bcheck - проверять-ли наличие параметра ппп в массиве для обновления 
	 * */
...
//		spartime = sdf.format(dt);
		...
		boolean gp_exist = st.isAppInstalled(inst, st.APP_GOOGLE_PLAY_PACKAGE);
...
//        	return;
		...
		if (path!= null) {
			st.runActShowText(inst, R.string.tpl_full_edit, path, ShowTextAct.FLAG_EXTERNAL_FILE_EDIT
...
//        }
        ...
        else if("kbd_background_pict".equals(k))
        {
            ...
            new DlgFileExplorer(inst, 
            		inst.getString(R.string.set_kbd_background),
            		DlgFileExplorer.PICTURE_EXT,
            		DlgFileExplorer.TYPE_PICTURE,
            		null,
            		txt,
...
//        int steps[] = new int[]{50,100,100};
        ...
        if (inst!=null) {
        	try {
            	title = inst.getString(R.string.set_key_intervals)
            			+st.STR_LF+"("+inst.getString(R.string.intervals_keys_title_def_value)
            			+title+")"+st.STR_LF;
...
//    }
    ...
    {
        Dlg.yesNoDialog(cont, cont.getString(bSave?R.string.set_key_save_pref:R.string.set_key_load_pref)+" ?", new st.UniObserver()
        {
            @Override
            public int OnObserver(Object param1, Object param2)
            {
